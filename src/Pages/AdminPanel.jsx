...
import {
  ...
  FiPackage, FiImage, FiSave, FiX, FiUpload, FiGrid, FiAlertTriangle, FiSettings,
} from "react-icons/fi";
import { db, storage } from "../firebase";
import {
  collection, addDoc, updateDoc, deleteDoc, doc, getDocs, serverTimestamp, query, orderBy, setDoc
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0a0f0d] via-[#111a14] to-[#0d1510]">
      ...
      <header className="relative border-b border-green-900/30 bg-[#111a14]/80 backdrop-blur-xl">
        <div className="mx-auto max-w-7xl flex flex-col md:flex-row items-center justify-between px-6 py-4 gap-4 md:gap-0">
          ...
          <div className="flex flex-wrap justify-center items-center gap-2 md:gap-4 w-full md:w-auto">
            <button 
              ...
              className={`flex items-center gap-2 text-sm px-4 py-2 rounded-lg transition border ${maintenanceMode ? 'bg-yellow-900/20 text-yellow-400 border-yellow-500/30 hover:bg-yellow-900/40' : 'bg-[#0d1510] text-gray-400 border-green-900/30 hover:bg-green-900/20 hover:text-white'}`}
              title={maintenanceMode ? "Disable Maintenance Mode" : "Enable Maintenance Mode"}
            >
              <FiSettings className={maintenanceMode ? "animate-[spin_3s_linear_infinite]" : ""} />
